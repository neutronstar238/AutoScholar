...
class TestTrendingManager:
    ...
    @pytest.mark.asyncio
    async def test_update_trending_paper_creates_new_record(self):
        """测试创建新的热门论文记录"""
    ...
    @pytest.mark.asyncio
    async def test_update_trending_paper_increments_count(self):
        """测试更新现有论文增加推荐计数"""
    ...
    @pytest.mark.asyncio
    async def test_get_trending_papers_returns_recent(self):
        """测试获取热门论文只返回最近推荐的"""
        # 创建一个最近推荐的论文
        recent_paper_id = f"recent_paper_{datetime.utcnow().timestamp()}"
        await trending_manager.update_trending_paper(
    ...
    @pytest.mark.asyncio
    async def test_get_trending_papers_by_category(self):
        """测试按分类获取热门论文"""
        # 创建不同分类的论文
        paper_id_ai = f"ai_paper_{datetime.utcnow().timestamp()}"
        paper_id_cv = f"cv_paper_{datetime.utcnow().timestamp()}"
    ...
    @pytest.mark.asyncio
    async def test_get_trending_by_category_multiple(self):
        """测试按多个分类获取热门论文"""
    ...
    @pytest.mark.asyncio
    async def test_trending_papers_sorted_by_score(self):
        """测试热门论文按评分排序"""
        # 创建多个论文并多次推荐其中一个
        paper_id_high = f"high_score_{datetime.utcnow().timestamp()}"
        paper_id_low = f"low_score_{datetime.utcnow().timestamp()}"
